...
import com.olshevchenko.onlineshop.repository.jdbc.JdbcProductRepository;
...
class JdbcProductRepositoryTest {
    ...
    private final JdbcProductRepository jdbcProductRepository = new JdbcProductRepository(jdbcTemplate);
    ...
    JdbcProductRepositoryTest() throws SQLException {
        ...
        BufferedReader bufferedReader = new BufferedReader(
                new InputStreamReader(Objects.requireNonNull(
                        JdbcProductRepositoryTest.class.getClassLoader().getResourceAsStream("products.sql"))));
    ...
    void testFindAllReturnList() {
        ...
        List<Product> actualProducts = jdbcProductRepository.findAll();
        assertEquals(expectedProducts, actualProducts);
    }

    @Test
    void testFindByIdReturnProduct() {
        Optional<Product> actualProduct = jdbcProductRepository.findById(1);
    ...
    void testAddAndUpdateAndRemove() {
        List<Product> actualProductsBefore = jdbcProductRepository.findAll();
        assertFalse(actualProductsBefore.contains(productNokia));
        jdbcProductRepository.add(productNokia);
        List<Product> actualProductsAfter = jdbcProductRepository.findAll();
        ...
        jdbcProductRepository.update(productNokia);
        assertEquals("Updated!!!", productNokia.getName());

        jdbcProductRepository.remove(4);
